...
- 파일명은 `V<번호>__<snake_case 설명>.sql`로 쓴다. 예: `V2__add_article_view_count.sql`
...
자식 테이블이 이미 FK를 갖고 있어서 양방향은 추가 작업일 뿐이다.
